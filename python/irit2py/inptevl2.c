...
{
    char *p, *q, CharData[100000];
    int i;

    GlblNumOfContCommas = GlblIndent = GlblUserDefFunc = 0;

    CharData[0] = 0;			   /* Make the string empty. */
    /* Copy to a local strings and then send to stdout. */
    LocalPrintTree(Root, 1, CharData, 100000);
    for (i = 0; i < strlen(CharData); i++) {
        if (isupper(CharData[i]))
	    CharData[i] = tolower(CharData[i]);
    }

    if (GlblUserDefFunc) {
	strcat(CharData, "\n");
        for (i = 0; i < GlblIndent; i++)
	    strcat(CharData, "    ");
	strcat(CharData, "return retval");
    }

    /* Do some special cases of pretty printing - irit.E?/P?. */
    p = CharData;
    while ((p = strstr(p, "irit.e")) != NULL) {
	if (isdigit(p[6]))
	    p[5] = 'E';
    ...
    /* Do some special cases of pretty printing - irit.ctlpt. */
    if ((p = strstr(CharData, "irit.ctlpt")) != NULL) {
        int IndentCtlpt = 0;

	for (q = p; q != CharData && *q != '\n' && *q != '\r'; q--)
	    IndentCtlpt++;

	if ((q = strstr(p + 1, "irit.ctlpt")) != NULL) {
	    *q = 0;
	    printf("%s\\\n", CharData);
	    *q = 'i';
	    p = q;

	    while ((q = strstr(p + 1, "irit.ctlpt")) != NULL) {
	        *q = 0;
		if (p != CharData) {
		    for (i = 0; i < IndentCtlpt; i++)
		        printf(" ");
		}
		printf("%s\\\n", p);
		*q = 'i';
	    ...
	    printf("%s\n", CharData);		          /* single ctlpt... */
    }
    else
        printf("%s\n", CharData);			     /* and print... */
}

void InptPrsrPrintTreeOrig(const ParseTree *Root, char *Str, int StrLen)
{
    if (Str == NULL) {
			   ...
			   int StrLen)
{
	...
	if (Str[Len - 1] == '.')
	    ...
	    fprintf(stderr, "Error: Irit2py found too long of an expression.  Py file will be truncated.\n");
	    strcat(Str, "...");
	    return;
	}
    }
...
#   endif /* DEBUG */
	...
	case IP_NUM_FUNC:
	    FName = NumFuncTable[Root -> NodeKind -
				 IP_NUM_FUNC_OFFSET].FuncName;
	    Level = 0;
	    GlblNumOfContCommas = 0;
	    CloseFlag = TRUE;
	    if (stricmp(FName, "acos") == 0 ||
	        stricmp(FName, "asin") == 0 ||
	        stricmp(FName, "atan2") == 0 ||
	        stricmp(FName, "atan") == 0 ||
	        stricmp(FName, "cos") == 0 ||
	        stricmp(FName, "sin") == 0 ||
	        stricmp(FName, "exp") == 0 ||
	        stricmp(FName, "pow") == 0 ||
	        stricmp(FName, "sqrt") == 0 ||
	        stricmp(FName, "tan") == 0 ||
	        stricmp(FName, "sin") == 0) {
	        strcat(Str, "math.");
		strcat(Str, FName);
	...
	case IP_OBJ_FUNC3:
	case IP_OBJ_FUNC4:
	    FName = ObjFuncTable[Root -> NodeKind -
				IP_OBJ_FUNC_OFFSET].FuncName;
	    Level = 0;
	    GlblNumOfContCommas = 0;
	    CloseFlag = TRUE;
	    if (stricmp(FName, "vector") == 0) {
	    }
	    else {
	        strcat(Str, IRITPY_PREFIX);
		strcat(Str, FName);
	...
	case IP_GEN_FUNC:
	    ...
	    GlblNumOfContCommas = 0;
	    CloseFlag = TRUE;
	    if (GenFuncTable[Root -> NodeKind - 
			     IP_GEN_FUNC_OFFSET].FuncToken == IP_FOR) {
	        /* While iterator init. */
	        LocalPrintTree(Root -> Right -> Left, Level, Str, StrLen);
		strcat(Str, "\n");
		for (j = 0; j < GlblIndent; j++)
		    strcat(Str, "    ");
		strcat(Str, "while ( ");
		LocalPrintTree(Root -> Right -> Left -> Left,
			       Level, Str, StrLen);
		strcat(Str, " <= ");
		LocalPrintTree(Root -> Right -> Right -> Right -> Left,
			       Level, Str, StrLen);
		strcat(Str, " ):\n");
		...
		for (j = 0; j < GlblIndent; j++)
		    strcat(Str, "    ");
		LocalPrintTree(Root-> Right -> Right -> Right -> Right,
			       Level, Str, StrLen);
		strcat(Str, "\n");
		for (j = 0; j < GlblIndent; j++)
		    strcat(Str, "    ");
		...
		LocalPrintTree(Root -> Right -> Left -> Left,
			       Level, Str, StrLen);
		strcat(Str, " + ");
		LocalPrintTree(Root -> Right -> Right -> Left,
			       Level, Str, StrLen);
		GlblIndent--;
		return;
	    }
	    else if (GenFuncTable[Root -> NodeKind - 
				  IP_GEN_FUNC_OFFSET].FuncToken == IP_WHILE) {
	        strcat(Str, "while ( ");
		LocalPrintTree(Root -> Right -> Left,
			       Level, Str, StrLen);
		strcat(Str, " ):\n");
		...
		for (j = 0; j < GlblIndent; j++)
		    strcat(Str, "    ");
		LocalPrintTree(Root -> Right -> Right,
			       Level, Str, StrLen);
		GlblIndent--;
		return;
	    }
	    else if (GenFuncTable[Root -> NodeKind - 
				  IP_GEN_FUNC_OFFSET].FuncToken == IP_IF) {
	        strcat(Str, "if ( ");
		LocalPrintTree(Root -> Right -> Left, Level, Str, StrLen);
	        strcat(Str, " ):\n");
		GlblIndent++;
		...
		for (j = 0; j < GlblIndent; j++)
		    strcat(Str, "    ");
		if (Root -> Right -> Right -> NodeKind == IP_TKN_COMMA) {
		    /* We have both and TRUE and FALSE statements. */
		    LocalPrintTree(Root -> Right -> Right -> Left,
				   Level, Str, StrLen);
		    strcat(Str, "\n");
		    if (Root -> Right -> Right -> Right != NULL) {
		        for (j = 0; j < GlblIndent - 1; j++)
			    strcat(Str, "    ");
			strcat(Str, "else:\n");
			for (j = 0; j < GlblIndent; j++)
			    strcat(Str, "    ");
			LocalPrintTree(Root -> Right -> Right -> Right,
				       Level, Str, StrLen);
		    }
		}
		else {
		    /* We have only TRUE statement. */
		    LocalPrintTree(Root -> Right -> Right,
				   Level, Str, StrLen);
		}
		...
		return;
	        ...
	        strcat(Str, IRITPY_PREFIX);
		strcat(Str,
		       GenFuncTable[Root -> NodeKind -
				    IP_GEN_FUNC_OFFSET].FuncName);
	...
	case IP_OPERATORS:
		...
		case IP_TKN_COLON:
		    if (Root -> Left -> NodeKind == IP_TKN_PARAMETER) {
		        /* An expression holding only a variable.  Probably */
		...
		case IP_TKN_EQUAL:
		    if (Root -> Right &&
			(Root -> Right -> NodeKind == IP_USERPROCDEF ||
			 Root -> Right -> NodeKind == IP_USERFUNCDEF)) {
		        strcat(Str, "def ");
			GlblUserDefOpenParan = TRUE;
			LocalPrintTree(Root -> Left, Level, Str, StrLen);
			GlblIndent++;
			if (Root -> Right -> NodeKind == IP_USERFUNCDEF)
			    GlblUserDefFunc = TRUE;
		    }
		    else {		    
		        LocalPrintTree(Root -> Left, Level, Str, StrLen);
			strcat(Str, " = ");
		...
		case IP_TKN_NUMBER:
		    if (Root -> PObj -> U.R == (double) CAGD_CONST_U_DIR)
		        strcat(Str, IRITPY_PREFIX "COL");
		    else if (Root -> PObj -> U.R == (double) CAGD_CONST_V_DIR)
		        strcat(Str, IRITPY_PREFIX "ROW");
		    else if (Root -> PObj -> U.R == (double) TRIV_CONST_W_DIR)
		        strcat(Str, IRITPY_PREFIX "DEPTH");

		    else if (Root -> PObj -> U.R == KV_UNIFORM_OPEN)
		        strcat(Str, IRITPY_PREFIX "KV_OPEN");
		    else if (Root -> PObj -> U.R == KV_UNIFORM_DISCONT_OPEN)
		        strcat(Str, IRITPY_PREFIX "KV_DISC_OPEN");
		    else if (Root -> PObj -> U.R == KV_UNIFORM_FLOAT)
		        strcat(Str, IRITPY_PREFIX "KV_FLOAT");
		    else if (Root -> PObj -> U.R == KV_UNIFORM_PERIODIC)
		        strcat(Str, IRITPY_PREFIX "KV_PERIODIC");

		    else if (Root -> PObj -> U.R == (double) CAGD_NIELSON_FOLEY_PARAM)
		        strcat(Str, IRITPY_PREFIX "PARAM_NIELFOL");
		    else if (Root -> PObj -> U.R == (double) CAGD_CHORD_LEN_PARAM)
		        strcat(Str, IRITPY_PREFIX "PARAM_CHORD");
		    else if (Root -> PObj -> U.R == (double) CAGD_CENTRIPETAL_PARAM)
		        strcat(Str, IRITPY_PREFIX "PARAM_CENTRIP");
		    else if (Root -> PObj -> U.R == (double) CAGD_UNIFORM_PARAM)
		        strcat(Str, IRITPY_PREFIX "PARAM_UNIFORM");

		    else if (Root -> PObj -> U.R == CAGD_PT_E1_TYPE)
		        strcat(Str, IRITPY_PREFIX "E1");
		    else if (Root -> PObj -> U.R == CAGD_PT_E2_TYPE)
		        strcat(Str, IRITPY_PREFIX "E2");
		    else if (Root -> PObj -> U.R == CAGD_PT_E3_TYPE)
		        strcat(Str, IRITPY_PREFIX "E3");
		    else if (Root -> PObj -> U.R == CAGD_PT_E4_TYPE)
		        strcat(Str, IRITPY_PREFIX "E4");
		    else if (Root -> PObj -> U.R == CAGD_PT_E5_TYPE)
		        strcat(Str, IRITPY_PREFIX "E5");
		    else if (Root -> PObj -> U.R == CAGD_PT_E6_TYPE)
		        strcat(Str, IRITPY_PREFIX "E6");
		    else if (Root -> PObj -> U.R == CAGD_PT_E7_TYPE)
		        strcat(Str, IRITPY_PREFIX "E7");
		    else if (Root -> PObj -> U.R == CAGD_PT_E8_TYPE)
		        strcat(Str, IRITPY_PREFIX "E8");
		    else if (Root -> PObj -> U.R == CAGD_PT_E9_TYPE)
		        strcat(Str, IRITPY_PREFIX "E9");
		    else if (Root -> PObj -> U.R == CAGD_PT_P1_TYPE)
		        strcat(Str, IRITPY_PREFIX "P1");
		    else if (Root -> PObj -> U.R == CAGD_PT_P2_TYPE)
		        strcat(Str, IRITPY_PREFIX "P2");
		    else if (Root -> PObj -> U.R == CAGD_PT_P3_TYPE)
		        strcat(Str, IRITPY_PREFIX "P3");
		    else if (Root -> PObj -> U.R == CAGD_PT_P4_TYPE)
		        strcat(Str, IRITPY_PREFIX "P4");
		    else if (Root -> PObj -> U.R == CAGD_PT_P5_TYPE)
		        strcat(Str, IRITPY_PREFIX "P5");
		    else if (Root -> PObj -> U.R == CAGD_PT_P6_TYPE)
		        strcat(Str, IRITPY_PREFIX "P6");
		    else if (Root -> PObj -> U.R == CAGD_PT_P7_TYPE)
		        strcat(Str, IRITPY_PREFIX "P7");
		    else if (Root -> PObj -> U.R == CAGD_PT_P8_TYPE)
		        strcat(Str, IRITPY_PREFIX "P8");
		    else if (Root -> PObj -> U.R == CAGD_PT_P9_TYPE)
		        strcat(Str, IRITPY_PREFIX "P9");

		    else if (Root -> PObj -> U.R == (double) FF_BEZIER_TYPE)
		        strcat(Str, IRITPY_PREFIX "BEZIER_TYPE");
		    else if (Root -> PObj -> U.R == (double) FF_BSPLINE_TYPE)
		        strcat(Str, IRITPY_PREFIX "BSPLINE_TYPE");
		    else if (Root -> PObj -> U.R == (double) FF_POWER_TYPE)
		        strcat(Str, IRITPY_PREFIX "POWER_TYPE");
		    else if (Root -> PObj -> U.R == (double) FF_GREGORY_TYPE)
		        strcat(Str, IRITPY_PREFIX "GREGORY_TYPE");
		...
		case IP_TKN_PARAMETER:
		    if (stricmp(Root -> PObj -> ObjName, "return") == 0)
		        sprintf(&Str[strlen(Str)], "retval");
		    else if (stricmp(Root -> PObj -> ObjName, "view_mat") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetViewMatrix()");
		    else if (stricmp(Root -> PObj -> ObjName, "prsp_mat") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetPrspMatrix()");
		    else if (stricmp(Root -> PObj -> ObjName, "resolution") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetResolution()");
		    else if (stricmp(Root -> PObj -> ObjName, "drawctlpt") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetDrawCtlpt()");
		    else if (stricmp(Root -> PObj -> ObjName, "flat4ply") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetFlat4Ply()");
		    else if (stricmp(Root -> PObj -> ObjName, "PolyApproxOpt") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetPolyApproxOpt()");
		    else if (stricmp(Root -> PObj -> ObjName, "PolyApproxUV") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetPolyApproxUV()");
		    else if (stricmp(Root -> PObj -> ObjName, "PolyApproxTri") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetPolyApproxTri()");
		    else if (stricmp(Root -> PObj -> ObjName, "PolyApproxTol") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetPolyApproxTol()");
		    else if (stricmp(Root -> PObj -> ObjName, "PolyMergeCoplanar") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetPolyMergeCoplanar()");
		    else if (stricmp(Root -> PObj -> ObjName, "Machine") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetMachine()");
		    else if (stricmp(Root -> PObj -> ObjName, "Axes") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetAxes()");
		    else if (stricmp(Root -> PObj -> ObjName, "UsrFnList") == 0)
		        sprintf(&Str[strlen(Str)], "irit.GetUsrFnList()");
		    else
...
static void LocalPrintTreeOrig(const ParseTree *Root,
			       int Level,
			       char *Str,
			       int StrLen)
{
...
void InptEvalPropagateDependencies(IPObjectStruct *PObj, ParseTree *Root)
{
	...
	*Dpnds = PObj -> Dpnds;
    ...
    /* lists.								     */
    InptPrsrPrintTreeOrig(Root, Str, IP_EXPR_MAX_STRING_LEN);
    strcat(Str, ";");
