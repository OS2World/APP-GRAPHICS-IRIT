...
;;;
;;; Internal function to make end of the irit-solid-modeller buffer visible.
(defun make-eob-irit-solid-modeller-visible ()
  (let* ((crnt-buffer (buffer-name)))
    (switch-to-buffer-other-window (get-buffer "irit-solid-modeller"))
    (goto-char (point-max))
    (recenter)
    (insert "\n");  ; For some reason we must print something to mak visible.
    (switch-to-buffer-other-window (get-buffer crnt-buffer))))
...
and may be changed."
  ...
  (if (equal major-mode 'irit-mode)
	...
	(goto-char (point-max))
	(make-eob-irit-solid-modeller-visible)
	(switch-to-buffer-other-window (get-buffer crnt-buffer))))
